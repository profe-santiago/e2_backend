  ...
  <AppLayout>
    <div style="max-width:64rem;margin:0 auto;padding-top:2rem">
      ...
      <div style="margin-bottom:2rem;display:flex;justify-content:space-between;align-items:center">
        <h2 style="font-size:1.5rem;font-weight:700;color:var(--text-primary)">Editar Carrera</h2>
...
async function fetchCarrera() {
  try {
    const res = await api.get(`/admin/carreras/${route.params.id}`)
    form.value = { nombre: res.data.data.nombre, clave: res.data.data.clave || '' }
  } catch (error) {
...
async function save() {
  try {
    await api.put(`/admin/carreras/${route.params.id}`, form.value)
