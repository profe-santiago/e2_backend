  ...
  <AppLayout>
    <div style="max-width:64rem;margin:0 auto">
      ...
      <div style="margin-bottom:2rem;display:flex;justify-content:space-between;align-items:center">
        <h2 style="font-size:1.5rem;font-weight:700;color:var(--text-primary)">Editar Usuario</h2>
      ...
      <!-- Contenedor -->
      <div v-if="fetching" class="loading" style="padding:4rem;text-align:center"><div class="spinner"></div></div>
      <div v-else style="background:var(--card-bg,#fff);border:1px solid var(--border,#e5e7eb);border-radius:.5rem;box-shadow:0 1px 3px rgba(0,0,0,.1);overflow:hidden">
        ...
        <form @submit.prevent="updateUser" style="padding:2rem">
          ...
          <!-- SECCIÓN 2: SEGURIDAD (Opcional en edición) -->
          <div style="margin-bottom:1.5rem;padding-bottom:.5rem;border-bottom:1px solid var(--border,#e5e7eb);display:flex;justify-content:space-between;align-items:center">
            <h4 style="font-weight:500;color:var(--text-primary)">Seguridad</h4>
            <span style="font-size:.75rem;color:var(--text-muted)">Deja en blanco para mantener la contraseña actual</span>
          </div>
...
onMounted(async () => {
  try {
    const res = await api.get(`/admin/usuarios/${userId}`)
    const user = res.data.data.usuario
    form.value.name = user.name
    form.value.email = user.email
    if (user.roles && user.roles.length > 0) {
      form.value.rol_id = user.roles[0].id
    }
  } catch (err) {
...
async function updateUser() {
  ...
  try {
    ...
    await api.put(`/admin/usuarios/${userId}`, payload)
    router.push({ name: 'Usuarios', query: { success: 'Usuario actualizado correctamente' } })
  } catch (err) {
